...
void defineBinning(Int_t   jetChannel = 0 , TString theSample = "WW", Bool_t jetGenVeto = 0 ) {
  ...
  //tree->Add(filesPath + "latino_002_WWJets_mcnlo_OF_genJets_Smear_BUG.root");
  ...
  //tree->Add(filesPath + "latino_000_WWJets_mad_OF_genJets_Smear_BUG.root");
  ...
  TString path = Form("_GEN_0jet_mad_full_NNLL_JetGenVeto.root"); 
  //TString path = Form("_pow_test.root");
  ...
  Float_t nllW;  tree->SetBranchAddress("nllW", &nllW);
...
// 
 ...
 //----------------------------------------------------------------------------
  ...
  //for (int ievent=Nentries; ievent<tree->GetEntriesFast(); ievent++) {
    ...
    Double_t totalWReco =  nllW * puW *  mybaseW * luminosity;// * effW * triggW ;//puW *  mybaseW * luminosity;//puW * effW * triggW * mybaseW * luminosity;
    ...
    /////////NOOOOOOOOOOOOOOOOO if ( jetGen1_pt < 10 && jetpt1 < 10) continue;
